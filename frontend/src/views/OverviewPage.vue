...
// compute rowsCount after filteredRows is available to avoid TDZ/runtime errors
  ...
  <div class="layout-shell">
    ...
    <div class="card-surface" style="margin-bottom: 1rem">
      <div class="flex align-items-center justify-content-between flex-column md:flex-row gap-3">
        <div>
          ...
          <h1 style="margin: 0.25rem 0 0.5rem">Overview</h1>
        </div>
        <div class="flex align-items-center gap-3 flex-wrap">
          <Tag severity="success" :value="`${rowsCount.toLocaleString()} rows`"/>
          <Tag :severity="settings.datasetMode === 'full' ? 'success' : 'info'"
               :value="settings.datasetMode ? (settings.datasetMode === 'full' ? 'Full dataset' : 'Sample') : 'No dataset'"/>
        </div>
      </div>
    </div>
    ...
    <div v-if="dataReady" class="grid-gap" style="margin-top: 1rem">
      <StatsBar :stats="stats"
                :format-currency="(number) => number.toLocaleString('de-DE', {style: 'currency', currency: 'EUR'})"
                :format-number="(number) => number.toFixed(0).toLocaleString()"/>
      <div class="card-surface">
        <h3>Timeline</h3>
        <TimelineChart :data="filteredRows"/>
      </div>
    </div>
